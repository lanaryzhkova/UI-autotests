...
class TestAccountPage:
    ...
    def test_not_valid_deposit(self, driver, customer_is_logged_in):
        ...
        assert not account_page.is_success_deposit(), (
    ...
    def test_not_valid_withdrawl(self, driver, customer_is_logged_in):
        ...
        assert account_page.is_failed_withdrawl(), (
            "Ожидалось, что транзакция пройдет не успешно"
        )
    ...
    def test_reset_transactions(self, driver, customer_with_transactions):
        ...
        assert count_before_reset == 6, (
            f"Количество транзакций не совпадает, получено {count_before_reset}"
        )
        account_page.reset_transactions()
        count_after_reset = account_page.count_transactions_after_reset()
        assert count_after_reset, (
            f"После удаления остались транзакции {count_after_reset}"
        )
    ...
    def test_search_and_delete_customer(self, driver, created_customer):
        ...
        assert banking_manager_login_page.search_customer(first_name, last_name), (
            "Клиент не найден в списке"
        )
        banking_manager_login_page.delete_customer(first_name, last_name)
        assert not banking_manager_login_page.search_customer(first_name, last_name), (
            "Клиент найден в списке, не удален"
        )
