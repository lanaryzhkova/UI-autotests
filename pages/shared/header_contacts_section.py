...
class HeaderContactsSection(BasePage):
    ...
    def assert_contact_links(self):
        ...
        assert set(contact_links) == set(expected_contact_links), (
            f"Ссылки {contact_links} не соответствуют ожидаемым ссылкам на контакты {expected_contact_links}"
        )

    def assert_social_links(self):
        """Проверка ссылок на соцсети"""
        social_links = self.get_social_links()
        assert set(social_links) == set(expected_social_links), (
            f"Ссылки {social_links} не соответствуют ожидаемым ссылкам на соцсети {expected_social_links}"
        )
